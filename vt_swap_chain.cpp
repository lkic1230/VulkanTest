...
namespace vt {
    ...
    VtSwapChain::VtSwapChain(
        VtDevice& deviceRef, VkExtent2D extent, std::shared_ptr<VtSwapChain> previous)
    ...
    VkResult VtSwapChain::submitCommandBuffers(const VkCommandBuffer* buffers, uint32_t* imageIndex) {
    ...
    void VtSwapChain::createColorResources()
    {
        ...
        for (int i = 0; i < colorImages.size(); i++)
        {
            device.createImage(swapChainExtent.width, swapChainExtent.height, 1, device.getMsaaSampleCount(), colorFormat, VK_IMAGE_TILING_OPTIMAL
                , VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT
                , VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
                colorImages[i], colorImageMemorys[i]);

            colorImageViews[i] = device.createImageView(colorImages[i], colorFormat, VK_IMAGE_ASPECT_COLOR_BIT, 1);
        }
    }
    ...
    void VtSwapChain::createDepthResources() {
        ...
        for (int i = 0; i < depthImages.size(); i++) 
        {
            device.createImage(swapChainExtent.width, swapChainExtent.height, 1, device.getMsaaSampleCount(), depthFormat, VK_IMAGE_TILING_OPTIMAL
                , VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, depthImages[i], depthImageMemorys[i]);

            depthImageViews[i] = device.createImageView(depthImages[i], depthFormat, VK_IMAGE_ASPECT_DEPTH_BIT, 1);
        }
    }
